...
Customer::Customer(char* customer_name, char* customer_email, char* customer_password, char* customer_address){
...
Customer* Customer::from_stream(redisReply* reply, int stream_num, int msg_num){
    ...
    for (int field_num = 2; field_num < ReadStreamMsgNumVal(reply, stream_num, msg_num); field_num +=  2) {
        ...
        if (!strcmp(key, "name")) {
            ...
            snprintf(address, PARAMETERS_LEN, "%s", value);
    ...
    return new Customer(name, mail, password, address);
}
